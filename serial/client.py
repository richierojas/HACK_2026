...
# Configuration
SERIAL_PORT = 'COM3'  # Change to 'COM3' on Windows, '/dev/ttyUSB0' for other boards
BAUD_RATE = 115200
        ...
        async with websockets.connect(WEBSOCKET_URL) as websocket:
            ...
            try:
                async with asyncio.TaskGroup() as tg:
                    # Write on or off message to serial if received
                    async def write_serial():
                        async for message in websocket:
                            ...
                            if (text == "on"):
                                print("Sent on message to serial")
                                ser.write(b"on\n")
                            elif (text == "off"):
                                print("Sent off message to serial")
                                ser.write(b"off\n")
                    ...
                    writeTask = tg.create_task(write_serial())
                    readTask = tg.create_task(read_serial())
            except* KeyboardInterrupt:
                print("\nStopping...")
            except* Exception as eg:
                print("Unhandled exceptions: ", eg.exceptions)
            finally:
                ser.close()
